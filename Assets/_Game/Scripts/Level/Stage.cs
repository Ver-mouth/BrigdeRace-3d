...
public enum ColorType { White, Blue, Brown, Green, Purple, Red, Yellow }

public class Stage : MonoBehaviour
{
    public Transform[] brickPoints;
    public List<Vector3> emptyPoint = new List<Vector3>();
    public List<Brick> liBrick = new List<Brick>();
    ...
    internal void RemoveBrick(Brick brick)
    {
        ...
        liBrick.Remove(brick);
    }

    internal Brick SeekBrickPoint(ColorType colorType)
    {
        Brick brick = null;
        for (int i = 0; i < liBrick.Count; i++)
        {
            if (liBrick[i].colorType == colorType)
            {
                brick = liBrick[i];
                break;
            }
